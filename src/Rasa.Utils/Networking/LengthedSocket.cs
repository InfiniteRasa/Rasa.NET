...
public class LengthedSocket
{
    ...
    public IPAddress RemoteAddress => (Socket.RemoteEndPoint as IPEndPoint)!.Address;
    ...
    public LengthedSocket(Socket? socket, SizeType sizeHeaderLen, bool countSize)
    {
    ...
    private static SocketAsyncEventArgs? GrabEventArgs()
    {
        if (_socketAsyncEventArgsPool == null)
            throw new Exception("GrabEventArgs() - The socket async event args pool is not yet initialized!");

        lock (_socketAsyncEventArgsPool)
            return _socketAsyncEventArgsPool.Count > 0 ? _socketAsyncEventArgsPool.Pop() : null;
    }

    private static void ReturnEventArgs(SocketAsyncEventArgs args)
    {
        if (_socketAsyncEventArgsPool == null)
            throw new Exception("ReturnEventArgs() - The socket async event args pool is not yet initialized!");
    ...
    private int ReadSize(BufferData data)
    {
        ...
        return SizeHeaderLength switch
        {
            SizeType.Char => headerSize + data[0],
            SizeType.Word => headerSize + BitConverter.ToInt16(BufferData.Buffer, data.BaseOffset),
            SizeType.Dword => headerSize + BitConverter.ToInt32(BufferData.Buffer, data.BaseOffset),
            _ => throw new NotImplementedException($"Only 1, 2 and 4 byte headers are supported! {SizeHeaderLength} is not!"),
        };
    }
